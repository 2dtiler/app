...
import type {
  ...
  PendingDefoldTilesetImportState,
  Tileset,
} from "@/types";
...
export function useDefoldTilesetImport(
  enabled: boolean,
  onImportResolved: (imported: Tileset[]) => void,
) {
  const [pendingImport, setPendingImport] =
    useState<PendingDefoldTilesetImportState | null>(null);
  ...
  const handleImportDefoldTileset = useCallback(async (preselectedFile?: File) => {
    ...
    try {
      ...
      const attempt = await prepareDefoldTilesetImport(file.name, [
        {
          path: file.name,
          data: rootData,
        },
      ...
      console.error("[Import Defold Tile Source] Failed:", error);
      alert(
        error instanceof Error
          ? error.message
          : "Failed to import Defold tile source.",
      );
      return false;
    }
  ...
  const handleResolveImport = useCallback(async () => {
    ...
    try {
      ...
      const attempt = await prepareDefoldTilesetImport(pendingImport.rootPath, [
        {
