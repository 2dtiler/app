...
test("wang rules resolve cardinal edge combinations", () => {
  const terrains = [
    {
      id: "terrain-land",
      name: "Land",
      ...
      patternTiles: {
        ...
        "wang-0f": { sx: 48, sy: 0, sw: 16, sh: 16 },
      },
    },
  ] as AutotileConfig["terrains"];
  ...
  const autotile: AutotileConfig = {
    ...
    preset: "wang-tiles",
    terrains,
    rules: buildPresetAutotileRules({ preset: "wang-tiles", terrains }),
  };
  ...
  const verticalRun = resolveAutotileWrites({
    autotile,
    baseTiles: {
      "1,0": createTileRef(0, 0),
      "1,2": createTileRef(0, 0),
  ...
  assert.deepEqual(isolated.get("1,1"), createTileRef(16, 0));
  assert.deepEqual(verticalRun.get("1,1"), createTileRef(32, 0));
  assert.deepEqual(surrounded.get("1,1"), createTileRef(48, 0));
});
